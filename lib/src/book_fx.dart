...
class _BookFxState extends State<BookFx> with SingleTickerProviderStateMixin {
  ...
  // 当前页面
  @override
  void didUpdateWidget(covariant BookFx oldWidget) {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: GestureDetector(
        ...
        onHorizontalDragStart: gestureOnPanDown,
        onHorizontalDragUpdate: gestureOnPanUpdate,
        onHorizontalDragEnd: gestureOnPanEnd,
      ...
      if (p.value.a != p.value.f) {
        if (p.value.a.y == p.value.f.y) {
          ...
          if (!p.value.b.x.isNaN) {
            ...
            Path pyy2 = Path();
            Path mPathB =
                Path.combine(PathOperation.intersect, mPathAB, mPath1);
            ...
            Offset u = Offset(
                PaperPoint.toTwoPoint(
                        p.value.a, p.value.f, p.value.d, p.value.i)
                    .x,
                PaperPoint.toTwoPoint(
                        p.value.a, p.value.f, p.value.d, p.value.i)
                    .y);
